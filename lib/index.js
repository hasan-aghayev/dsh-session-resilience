		...
		this.routeDisposers.push(this.ctx.webServer.register({
			...
			handler: (req, res) => {
				if (!isLoopbackRequest(req)) {
					json(res, 403, { ok: false, error: "local requests only" });
					return;
				}
				json(res, 200, {
					ok: true,
	...
	ctx.inject([
		...
		bridgeRouteDisposers.push(engineCtx.webServer.register({
			...
			handler: (req, res) => {
				...
				if (!isLoopbackRequest(req)) {
					jsonResponse(res, 403, { ok: false, error: "local requests only" });
					return;
				}
				res.writeHead(200, {
		...
		bridgeRouteDisposers.push(engineCtx.webServer.register({
			...
			handler: (req, res) => {
				...
				if (!trustedLoopback(req)) {
					jsonResponse(res, 403, { ok: false, error: "untrusted origin" });
					return;
				}
				let body = "";
				let rejected = false;
