...
export interface AutoContinueSettingsCardState extends CardShell {
  recoveryPreset: CardFieldState;
  enabled: CardFieldState;
  restartAutoContinue: CardFieldState;
  restartResumeWindowMs: CardFieldState;
...
export class AutoContinueSettingsCardController {
  ...
  constructor(scope: SettingsScope<AutoContinueSettings>) {
    this.form = new CardForm(scope, [
      enumField('recoveryPreset', ['manual', 'safe', 'balanced', 'long-task']),
      booleanField('enabled'),
      booleanField('restartAutoContinue'),
      numberField('restartResumeWindowMs', 60_000),
      booleanField('paused'),
...
function ChoiceField(props: FieldProps & { options: readonly { value: string; label: string }[] }) {
  return (
    <div className={props.wide === true ? 'dshAcField dshAcFieldWide' : 'dshAcField'}>
      <div className="dshAcHead">
        <label className="dshAcLabel" htmlFor={props.id}>{props.label}</label>
      ...
      <p className={props.invalid ? 'dshAcInvalid' : 'dshAcHint'}>
        {props.invalid ? props.t('chrome.invalidChoice') : props.hint}
      </p>
    </div>
  );
}
      ...
      <div className="dshAcFormCanvas">
        ...
        >
          <ChoiceField
            ...
            options={[
              { value: 'safe', label: t('preset.safe') },
              { value: 'balanced', label: t('preset.balanced') },
              { value: 'long-task', label: t('preset.longTask') },
              { value: 'manual', label: t('preset.manual') },
            ]}
            onEdit={(text) => {
              props.edit('recoveryPreset', text);
              if (text !== '' && text !== 'manual') {
                for (const field of PRESET_FIELDS) props.resetField(field);
              }
            }}
