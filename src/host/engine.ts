...
export class AutoContinueRunner {
  ...
  constructor(
    ...
    this.disposeSessionEvents = ctx.on('session/event', (session, event) => {
      try {
        ...
        console.error(
          `[dsh-session-resilience] 会话事件处理异常 ${session.id}: ${
  ...
  private buildContinueText(
    ...
    const context: TemplateContext = {
      ...
      ...(state.lastFailure !== undefined ? { facts: state.lastFailure } : {}),
      ...(tool !== undefined ? { tool } : {}),
      ...(turn !== undefined ? { turn } : {}),
      ...(state.lastFailureAt > 0 ? { elapsedMs: Date.now() - state.lastFailureAt } : {}),
    };
    let text = fillTemplate(template, context);
    if (!config.guardTools) return text;
    const guard = this.currentGuard(state);
    if (guard.kind === 'pending') {
      text += ` ${fillTemplate(config.guardPendingText, {
        ...(guard.tool !== undefined ? { tool: guard.tool } : {}),
        ...(guard.result !== undefined ? { result: guard.result } : {}),
      })}`;
    } else if (guard.kind === 'done') {
      text += ` ${fillTemplate(config.guardDoneText, {
        ...(guard.tool !== undefined ? { tool: guard.tool } : {}),
        ...(guard.result !== undefined ? { result: guard.result } : {}),
      })}`;
    }
