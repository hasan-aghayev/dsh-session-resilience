...
export type RecoveryPreset = 'manual' | 'safe' | 'balanced' | 'long-task';

/** Tunables controlled together by a named recovery policy. */
export interface RecoveryPresetValues {
  restartResumeWindowMs: number;
  graceMs: number;
  cooldownMs: number;
...
export const RECOVERY_PRESETS: Record<Exclude<RecoveryPreset, 'manual'>, RecoveryPresetValues> = {
  safe: {
  ...
  balanced: {
    restartResumeWindowMs: 10 * 60 * 1000,
    graceMs: 3000,
    cooldownMs: 20 * 1000,
    maxConsecutive: 3,
    scanLimit: 8,
  ...
  'long-task': {
    restartResumeWindowMs: 30 * 60 * 1000,
    graceMs: 5000,
    cooldownMs: 60 * 1000,
    maxConsecutive: 8,
    scanLimit: 16,
...
export interface AutoContinueSettings {
  ...
  recoveryPreset?: RecoveryPreset;
  /** Master switch for every automatic continuation path. */
  enabled?: boolean;
  /** Whether a deliberate host restart should resume the sessions it interrupted. */
  restartAutoContinue?: boolean;
  /** Time window in which a browser reconnect may receive the restart handoff. */
  restartResumeWindowMs?: number;
...
export function resolveConfig(section: AutoContinueSettings | undefined): AutoContinueConfig {
  const value = section ?? {};
  const recoveryPreset: RecoveryPreset =
    value.recoveryPreset === 'safe' || value.recoveryPreset === 'balanced' || value.recoveryPreset === 'long-task' || value.recoveryPreset === 'manual'
      ? value.recoveryPreset
      : DEFAULT_CONFIG.recoveryPreset;
  const preset = recoveryPreset === 'manual' ? undefined : RECOVERY_PRESETS[recoveryPreset];
  const recoveryNumber = (field: keyof RecoveryPresetValues, fallback: number): number =>
    preset?.[field] ?? numberOr(value[field], fallback);
...
export class ToolInvocationTracker {
  ...
  recordResult(event: SessionEvent<'tool/result'>): ToolRepeatSignal | undefined {
    ...
    if (typeof surfaceOp === 'object' && surfaceOp !== null) {
      ...
      if (
        surfaceOp.startSeq !== surfaceOp.endSeq ||
        call === undefined ||
        call.result === undefined ||
        call.resultSeq !== surfaceOp.startSeq
      ) {
        this.breakCorrelation(this.latest);
        return undefined;
      }
